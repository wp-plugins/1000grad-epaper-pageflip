    ...
    function epaperShortcode( $atts )
    {
        ...
        if ( strlen( $title ) != 0 ) {
            ...
            switch ( $res['ep_view_as'] ) {
                case 'Link':
                    $epaperHTML = "<a class=\"" . $css_class . "\" onclick=\"Shadowbox.open({content:'" . $res['ep_link'] . "index.html?closeAction=parent.Shadowbox.close',player:'iframe',title:'" . $res['ep_name'] . "'});\" 
                        href=\"javascript:;\">" . $res['ep_text_link'] . "</a>";
                    break;

                case 'Animiertes GIF':
                    $epaperHTML = "<a class=\"" . $css_class . "\" onclick=\"Shadowbox.open({content:'" . $res['ep_link'] . "index.html?closeAction=parent.Shadowbox.close',player:'iframe',title:'" . $res['ep_name'] . "'});\" 
                        href=\"javascript:;\"> <img src=\"" . $res['ep_link'] . "epaper/epaper-ani.gif\" alt=\"epaper preview gif\" border=\"0\" /> </a>";
                    break;

                case 'Grafik':
                    $epaperHTML = "<a class=\"" . $css_class . "\" onclick=\"Shadowbox.open({content:'" . $res['ep_link'] . "index.html?closeAction=parent.Shadowbox.close',player:'iframe',title:'" . $res['ep_name'] . "'});\" 
                        href=\"javascript:;\"> <img src=\"" . $res['ep_link'] . "epaper/preview.jpg\" alt=\"epaper cover\" border=\"0\" /> </a>";
                    break;
            }
        }
        return $epaperHTML;
